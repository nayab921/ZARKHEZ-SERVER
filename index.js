          ...
          if (controlMode === "auto") {
            ...
            if (now - lastWarningTime > 10000) {
              ...
              if (manualNotifCount < 2) {
                manualNotifCount++;
                console.log(`⚠️ WARNING (${manualNotifCount}/2) SENT (MANUAL MODE)`);
                sendPush(activeUser, `⚠️ WARNING (${manualNotifCount}/2)`, `Critical: ${reason}. Please turn off motor!`);
              } else {
                console.log("🛑 FORCED SHUTDOWN (MANUAL MODE)");
                await db.collection("iot_data").doc("relay").update({ command: "off", status: "off" });
                sendPush(activeUser, "🛑 FORCED STOP", "Motor stopped after 2 ignored warnings.");
                manualNotifCount = 0;
                lastWarningTime = 0;
              }
            }
    ...
    pendingSchedules.forEach(async (doc) => {
      ...
      if (sch.status === "pending") {
        ...
        if (isStartDay && sch.startMinutes === currentMins) {
          ...
          await db.collection("iot_data").doc("relay").update({
            command: "on", status: "on", activeUser: user, mode: "auto"
          });
