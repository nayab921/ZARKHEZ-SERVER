...
// =========================================================
// 🔔 SMART PUSH NOTIFICATION FUNCTION
// =========================================================
...
db.collection('iot_data').doc('relay').onSnapshot(async doc => {
    ...
    if (status === 'off') {
        if (activeSessionId) {
            ...
            // BILLING GENERATE KARNA (App jaisi Dates ke sath)
                ...
                setTimeout(async () => {
                    try {
                        ...
                        if (!billGenerated) { 
                            ...
                            // 👈 Timestamp.now() is used to match App frontend format exactly
                            ...
                            console.log(`💰 Bill Created! Rs ${billAmount.toFixed(2)} (Start/End Time added)`);
